...
RSpec.describe SassListen::Event::Loop do
  let(:config) { instance_double(SassListen::Event::Config, 'config') }
  let(:processor) { instance_double(SassListen::Event::Processor, 'processor') }
  ...
  before do
    ...
    allow(SassListen::Event::Processor).to receive(:new).with(config, reasons).
      and_return(processor)

    allow(SassListen::Internals::ThreadPool).to receive(:add) do |*args, &block|
      fail 'Unstubbed call:'\
    ...
    allow(SassListen::Internals::ThreadPool).to receive(:add) do |*_, &block|
    ...
    context 'when resume is called' do
      it 'fails' do
        expect { subject.resume }.
          to raise_error(SassListen::Event::Loop::Error::NotStarted)
      end
    end
